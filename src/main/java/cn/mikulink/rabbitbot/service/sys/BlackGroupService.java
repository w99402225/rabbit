 ...
 */
@Service
public class BlackGroupService {
    private Logger logger = LoggerFactory.getLogger(BlackGroupService.class);
    ...
    public String getFilePath() {
        return configPath + File.separator + "black_group";
    }

    public void loadFile() {
        try {
            ConstantBlackGroup.BLACK_GROUP = this.loadList();
        } catch (Exception ex) {
    ...
    public List<Long> removeBlack(List<Long> pids) throws IOException {
        ...
        //覆写原本配置
        //先读取出所有id，判重后直接覆写文件
        List<Long> tempBlackGroup = loadList();

        //过滤重复
        for (Long gid : pids) {
            if (!tempBlackGroup.contains(gid)) {
                continue;
            }
            tempBlackGroup.remove(gid);
        }
        //创建写入流
        BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(this.getFilePath(), false)));
        for (Long gid : tempBlackGroup) {
            out.write("\r\n" + gid);
        }
        //关闭写入流
        out.close();

        return tempBlackGroup;
    }
}
